import '../models/enums.dart';
import '../models/skill.dart';

/// 15个武功
final skills = <String, Skill>{
  'basic_fist': const Skill(
    id: 'basic_fist',
    name: '基础拳法',
    description: '最基本的拳法招式，打起来像挠痒痒。',
    type: SkillType.attack,
    quality: SkillQuality.crude,
    baseDamage: 8,
    damageMultiplier: 1.0,
    mpCost: 0,
  ),
  'basic_kick': const Skill(
    id: 'basic_kick',
    name: '基础腿法',
    description: '简单的踢击技巧，威力比拳法稍大。',
    type: SkillType.attack,
    quality: SkillQuality.crude,
    baseDamage: 10,
    damageMultiplier: 1.1,
    mpCost: 3,
  ),
  'iron_palm': const Skill(
    id: 'iron_palm',
    name: '铁砂掌',
    description: '以铁砂锤炼双掌，掌力刚猛沉重。',
    type: SkillType.attack,
    quality: SkillQuality.refined,
    baseDamage: 18,
    damageMultiplier: 1.3,
    mpCost: 8,
    requiredLevel: 3,
    learnCost: 50,
  ),
  'gale_sword': const Skill(
    id: 'gale_sword',
    name: '疾风剑法',
    description: '剑势如风，快若流星。讲究一个"快"字。',
    type: SkillType.attack,
    quality: SkillQuality.refined,
    baseDamage: 15,
    damageMultiplier: 1.4,
    mpCost: 10,
    buffSpeed: 3,
    buffDuration: 2,
    requiredLevel: 5,
    learnCost: 80,
  ),
  'shadow_strike': const Skill(
    id: 'shadow_strike',
    name: '暗影突刺',
    description: '从意想不到的角度发动攻击，防不胜防。',
    type: SkillType.hidden,
    quality: SkillQuality.refined,
    baseDamage: 20,
    damageMultiplier: 1.5,
    mpCost: 12,
    requiredLevel: 6,
    learnCost: 100,
  ),
  'mountain_palm': const Skill(
    id: 'mountain_palm',
    name: '开山掌',
    description: '凝聚全身内力于一掌，如同劈山裂石。',
    type: SkillType.attack,
    quality: SkillQuality.superior,
    baseDamage: 30,
    damageMultiplier: 1.6,
    mpCost: 18,
    requiredLevel: 10,
    learnCost: 200,
  ),
  'jade_bamboo_sword': const Skill(
    id: 'jade_bamboo_sword',
    name: '翠竹剑意',
    description: '以竹林之韵入剑，剑势飘逸灵动。柔中带刚，刚柔并济。',
    type: SkillType.attack,
    quality: SkillQuality.superior,
    baseDamage: 25,
    damageMultiplier: 1.7,
    mpCost: 15,
    buffAtk: 5,
    buffSpeed: 3,
    buffDuration: 3,
    requiredLevel: 8,
    learnCost: 180,
  ),
  'tuna_breathing': const Skill(
    id: 'tuna_breathing',
    name: '吐纳术',
    description: '最基础的呼吸吐纳之法，调息养气。',
    type: SkillType.innerForce,
    quality: SkillQuality.crude,
    baseDamage: 0,
    mpCost: 0,
    healAmount: 15,
  ),
  'golden_bell': const Skill(
    id: 'golden_bell',
    name: '金钟罩',
    description: '运气护体，如罩金钟。短时间内大幅提高防御。',
    type: SkillType.innerForce,
    quality: SkillQuality.refined,
    baseDamage: 0,
    mpCost: 12,
    buffDef: 10,
    buffDuration: 3,
    requiredLevel: 5,
    learnCost: 80,
  ),
  'qi_surge': const Skill(
    id: 'qi_surge',
    name: '真气涌动',
    description: '催动体内真气循环，提升攻击力。',
    type: SkillType.innerForce,
    quality: SkillQuality.refined,
    baseDamage: 0,
    mpCost: 10,
    buffAtk: 8,
    buffDuration: 3,
    requiredLevel: 4,
    learnCost: 60,
  ),
  'spring_return': const Skill(
    id: 'spring_return',
    name: '回春诀',
    description: '以内力温养经脉，恢复气血。医武双修的典型功法。',
    type: SkillType.innerForce,
    quality: SkillQuality.superior,
    baseDamage: 0,
    mpCost: 15,
    healAmount: 40,
    requiredLevel: 8,
    learnCost: 150,
  ),
  'swallow_dart': const Skill(
    id: 'swallow_dart',
    name: '燕子穿林',
    description: '以暗器模仿燕子穿林之姿，角度刁钻。',
    type: SkillType.hidden,
    quality: SkillQuality.refined,
    baseDamage: 12,
    damageMultiplier: 1.3,
    mpCost: 6,
    requiredLevel: 3,
    learnCost: 40,
  ),
  'mist_step': const Skill(
    id: 'mist_step',
    name: '踏雾步',
    description: '步法轻盈如踏雾而行，大幅提升身法。',
    type: SkillType.movement,
    quality: SkillQuality.refined,
    baseDamage: 0,
    mpCost: 8,
    buffSpeed: 8,
    buffDuration: 3,
    requiredLevel: 5,
    learnCost: 80,
  ),
  'chess_insight': const Skill(
    id: 'chess_insight',
    name: '观棋悟道',
    description: '从棋道中感悟天地之理，永久提升悟性。这是一种特殊的被动领悟。',
    type: SkillType.passive,
    quality: SkillQuality.superior,
    baseDamage: 0,
    mpCost: 0,
    requiredLevel: 1,
    learnCost: 0,
  ),
  'moongazing_art': const Skill(
    id: 'moongazing_art',
    name: '望月心法',
    description: '月夜修炼的内功心法，以月华滋养经脉。修炼时内力恢复速度大增。',
    type: SkillType.innerForce,
    quality: SkillQuality.ultimate,
    baseDamage: 0,
    mpCost: 0,
    healAmount: 25,
    buffAtk: 5,
    buffDef: 5,
    buffDuration: 5,
    requiredLevel: 12,
    learnCost: 300,
  ),
  // —— 被动武技 ——
  'passive_follow_fist': const Skill(
    id: 'passive_follow_fist',
    name: '后手追拳',
    description: '攻击后顺势补上一拳，打出追击伤害。',
    type: SkillType.passive,
    quality: SkillQuality.crude,
    baseDamage: 5,
    mpCost: 0,
    requiredLevel: 1,
    learnCost: 0,
  ),
  'passive_sweep_kick': const Skill(
    id: 'passive_sweep_kick',
    name: '下盘横扫',
    description: '趁敌不备扫腿攻下盘，常令对手失去重心。',
    type: SkillType.passive,
    quality: SkillQuality.refined,
    baseDamage: 8,
    mpCost: 0,
    requiredLevel: 3,
    learnCost: 40,
  ),
  'passive_elbow_strike': const Skill(
    id: 'passive_elbow_strike',
    name: '寸肘',
    description: '近身时以肘尖发力，短促而凶狠。',
    type: SkillType.passive,
    quality: SkillQuality.refined,
    baseDamage: 10,
    mpCost: 0,
    requiredLevel: 5,
    learnCost: 60,
  ),
  'passive_iron_body': const Skill(
    id: 'passive_iron_body',
    name: '铁布衫',
    description: '以内力护体，受击时自动反震伤害。',
    type: SkillType.passive,
    quality: SkillQuality.superior,
    baseDamage: 12,
    mpCost: 0,
    requiredLevel: 8,
    learnCost: 120,
  ),
  'passive_palm_strike': const Skill(
    id: 'passive_palm_strike',
    name: '掌风余力',
    description: '出招收势之间掌风自发，可在攻击后追加一掌。',
    type: SkillType.passive,
    quality: SkillQuality.crude,
    baseDamage: 6,
    mpCost: 0,
    requiredLevel: 1,
    learnCost: 20,
  ),
  'passive_finger_flick': const Skill(
    id: 'passive_finger_flick',
    name: '弹指',
    description: '手指蓄劲弹出，看似不经意，实则暗藏杀机。',
    type: SkillType.passive,
    quality: SkillQuality.refined,
    baseDamage: 7,
    mpCost: 0,
    requiredLevel: 4,
    learnCost: 50,
  ),
  'passive_hidden_needle': const Skill(
    id: 'passive_hidden_needle',
    name: '袖底飞针',
    description: '袖中暗藏细针，出手时趁隙掷出，防不胜防。',
    type: SkillType.passive,
    quality: SkillQuality.refined,
    baseDamage: 8,
    mpCost: 0,
    requiredLevel: 4,
    learnCost: 50,
  ),
  'passive_sword_qi': const Skill(
    id: 'passive_sword_qi',
    name: '剑气余韵',
    description: '剑招施出之后剑气不散，裹挟余劲追击对手。',
    type: SkillType.passive,
    quality: SkillQuality.refined,
    baseDamage: 9,
    mpCost: 0,
    requiredLevel: 5,
    learnCost: 70,
  ),
  'passive_blade_wind': const Skill(
    id: 'passive_blade_wind',
    name: '刀风斩',
    description: '刀势凌厉，挥刀之余卷起劲风割伤对手。',
    type: SkillType.passive,
    quality: SkillQuality.refined,
    baseDamage: 9,
    mpCost: 0,
    requiredLevel: 6,
    learnCost: 80,
  ),
  'passive_counter_punch': const Skill(
    id: 'passive_counter_punch',
    name: '借力打力',
    description: '巧妙借用对方招式的力道反弹回去，四两拨千斤。',
    type: SkillType.passive,
    quality: SkillQuality.superior,
    baseDamage: 11,
    mpCost: 0,
    requiredLevel: 7,
    learnCost: 100,
  ),
  'passive_shadow_step': const Skill(
    id: 'passive_shadow_step',
    name: '踏影追击',
    description: '身法诡异如影随形，踏着对手影子追出致命一击。',
    type: SkillType.passive,
    quality: SkillQuality.superior,
    baseDamage: 10,
    mpCost: 0,
    requiredLevel: 8,
    learnCost: 120,
  ),
  'passive_qi_burst': const Skill(
    id: 'passive_qi_burst',
    name: '真气爆发',
    description: '攻击瞬间内力外泄形成爆裂冲击，附带强烈的真气震荡。',
    type: SkillType.passive,
    quality: SkillQuality.ultimate,
    baseDamage: 15,
    mpCost: 0,
    requiredLevel: 10,
    learnCost: 200,
  ),

  // —— 师门技能 ——
  'tianjian_basic': const Skill(
    id: 'tianjian_basic',
    name: '天剑基础剑法',
    description: '天剑门入门剑法，剑势凌厉，攻守兼备。',
    type: SkillType.attack,
    quality: SkillQuality.refined,
    baseDamage: 22,
    damageMultiplier: 1.5,
    mpCost: 12,
    buffSpeed: 5,
    buffDuration: 2,
    requiredLevel: 8,
    learnCost: 0,
  ),
  'tianjian_advanced': const Skill(
    id: 'tianjian_advanced',
    name: '天剑进阶剑法',
    description: '天剑门核心剑法，剑意通明，威力惊人。',
    type: SkillType.attack,
    quality: SkillQuality.superior,
    baseDamage: 35,
    damageMultiplier: 1.8,
    mpCost: 20,
    buffAtk: 8,
    buffSpeed: 5,
    buffDuration: 3,
    requiredLevel: 15,
    learnCost: 0,
  ),
  'tianjian_ultimate': const Skill(
    id: 'tianjian_ultimate',
    name: '天剑绝学·剑心通明',
    description: '天剑门不传之秘，剑心通明，人剑合一。',
    type: SkillType.attack,
    quality: SkillQuality.divine,
    baseDamage: 50,
    damageMultiplier: 2.2,
    mpCost: 30,
    buffAtk: 15,
    buffSpeed: 10,
    buffDuration: 4,
    requiredLevel: 25,
    learnCost: 0,
  ),
  'luoxia_blade_basic': const Skill(
    id: 'luoxia_blade_basic',
    name: '落霞刀法',
    description: '落霞刀派基础刀法，刀势如落霞般绚烂。',
    type: SkillType.attack,
    quality: SkillQuality.refined,
    baseDamage: 25,
    damageMultiplier: 1.6,
    mpCost: 14,
    buffAtk: 5,
    buffDuration: 2,
    requiredLevel: 6,
    learnCost: 0,
  ),
  'luoxia_blade_advanced': const Skill(
    id: 'luoxia_blade_advanced',
    name: '落霞绝刀',
    description: '落霞刀派绝学，刀光如霞，势不可挡。',
    type: SkillType.attack,
    quality: SkillQuality.ultimate,
    baseDamage: 45,
    damageMultiplier: 2.0,
    mpCost: 25,
    buffAtk: 12,
    buffDef: 5,
    buffDuration: 3,
    requiredLevel: 18,
    learnCost: 0,
  ),
  'qingfeng_fist_basic': const Skill(
    id: 'qingfeng_fist_basic',
    name: '清风拳法',
    description: '清风拳门基础拳法，刚柔并济。',
    type: SkillType.attack,
    quality: SkillQuality.refined,
    baseDamage: 20,
    damageMultiplier: 1.4,
    mpCost: 10,
    buffDef: 5,
    buffDuration: 2,
    requiredLevel: 5,
    learnCost: 0,
  ),
  'iron_body': const Skill(
    id: 'iron_body',
    name: '铁布衫心法',
    description: '清风拳门护体功法，以内力护体，防御大增。',
    type: SkillType.innerForce,
    quality: SkillQuality.superior,
    baseDamage: 0,
    mpCost: 15,
    buffDef: 15,
    buffDuration: 4,
    requiredLevel: 10,
    learnCost: 0,
  ),
  'scholar_insight': const Skill(
    id: 'scholar_insight',
    name: '文士心法',
    description: '望月文士内功心法，以文入武，内力深厚。',
    type: SkillType.innerForce,
    quality: SkillQuality.superior,
    baseDamage: 0,
    mpCost: 12,
    healAmount: 30,
    buffSpeed: 8,
    buffDuration: 3,
    requiredLevel: 8,
    learnCost: 0,
  ),
};

/// 被动武技触发时的随机描述，{target} 会被替换为目标名称
const passiveHitFlavors = <String, List<String>>{
  'passive_follow_fist': [
    '顺手补了一拳，正中{target}肩头',
    '左手一探，打在{target}面门上',
    '拳风未散，又一记直拳轰向{target}胸口',
  ],
  'passive_sweep_kick': [
    '下盘一扫，踢在{target}小腿上',
    '侧身一脚，蹬在{target}腰间',
    '顺势一绊，{target}脚下踉跄',
  ],
  'passive_elbow_strike': [
    '近身发肘，捣在{target}肋下',
    '侧身一靠，肘尖顶在{target}胸口',
    '贴身短打，寸肘击中{target}腹部',
  ],
  'passive_iron_body': [
    '真气外放，震得{target}手臂发麻',
    '护体罡气反弹而出，{target}吃了个暗亏',
    '内力护体自动反击，{target}被震退半步',
  ],
  'passive_palm_strike': [
    '掌风一卷，拍在{target}后背',
    '顺手一掌，推在{target}胸前',
    '掌力未歇，又补了{target}一记',
  ],
  'passive_finger_flick': [
    '指尖一弹，点在{target}手腕穴位上',
    '顺势弹出一指，击中{target}咽喉',
    '手指轻弹如拨弦，正中{target}太阳穴',
  ],
  'passive_hidden_needle': [
    '袖中飞针射出，扎在{target}肩头',
    '暗针趁隙掷出，没入{target}小臂',
    '几枚细针如流星般飞向{target}',
  ],
  'passive_sword_qi': [
    '剑气余韵未消，划过{target}面颊',
    '一缕剑气追出，割在{target}衣袖上',
    '残留剑气如丝，掠过{target}身侧',
  ],
  'passive_blade_wind': [
    '刀风卷起，割在{target}手臂上',
    '刀势余劲横扫，{target}皮开肉绽',
    '刀气如风，在{target}身上又添一道口子',
  ],
  'passive_counter_punch': [
    '借力一带，把{target}自己的劲道弹了回去',
    '顺着{target}的攻势一引一送，对手反吃一拳',
    '四两拨千斤，{target}的力道反噬自身',
  ],
  'passive_shadow_step': [
    '脚步诡异地闪到{target}身后，一击即中',
    '踏着{target}的影子欺身而上，攻其不备',
    '身形如鬼魅般绕到{target}侧翼追击',
  ],
  'passive_qi_burst': [
    '内力猛然爆发，震得{target}口吐鲜血',
    '真气冲体而出，{target}被气浪掀退数步',
    '一股狂暴真气轰向{target}，地面碎石飞溅',
  ],
};

/// 内功被动效果定义
class InnerForcePassiveEffect {
  final int healHp;
  final int healMp;
  final int extraDamage;
  final String flavorText;

  const InnerForcePassiveEffect({
    this.healHp = 0,
    this.healMp = 0,
    this.extraDamage = 0,
    required this.flavorText,
  });
}

const innerForcePassives = <String, InnerForcePassiveEffect>{
  'tuna_breathing': InnerForcePassiveEffect(
    healMp: 5,
    flavorText: '吐纳之间真气流转，恢复了5点内力',
  ),
  'golden_bell': InnerForcePassiveEffect(
    healHp: 5,
    flavorText: '金钟罩余韵护体，恢复了5点气血',
  ),
  'qi_surge': InnerForcePassiveEffect(
    extraDamage: 5,
    flavorText: '真气涌动，拳劲暴增！',
  ),
  'spring_return': InnerForcePassiveEffect(
    healHp: 8,
    flavorText: '回春真气温养经脉，恢复了8点气血',
  ),
  'moongazing_art': InnerForcePassiveEffect(
    healHp: 5,
    healMp: 3,
    flavorText: '月华真气流转全身，气血内力微有恢复',
  ),
};
